...
const lastprojectId = () => {
  /* JSON 파일내에 마지막 포스트 projectId 를 가져옴 */
  // 가져오기전 캐싱된 데이터를 가져오지 않도록 새로 불러옴
  getData();
  return postData[postData.length - 1].projectId;
};
const refresh = (data) => {
...
export const Create = (req, res) => {
  // 클라이언트의 req body
  const { processStatus, title, section, body, author } = req.body;
  const dateNow = new Date();

  // projectId 에 들어갈 let 카운터
  // projectId 1씩 증가카운팅, 작성할때 현재한국시간으로 작성일 기입

  let postNum = lastprojectId() + 1;
  const post = {
    projectId: postNum,
    processStatus,
    title,
  ...
  // 카운팅된 projectId, 현재시각인 wrDate 가 추가.
  postData.push(post);
...
export const read = (req, res) => {
  //url 상으로 전달받은 클라이언트의 projectId read req
  const { projectId } = req.params;

  // 일치하는 post 찾기
  // 파라미터로 받아온 값은 비교할 문자 혹은 숫자.
  // objElem.projectId 값을 변환하여 사용해야함
  const post = postData.find(
    (objElem) => objElem.projectId.toString() === projectId,
  );
...
export const update = (req, res) => {
  //url 상으로 전달받은 클라이언트의 projectId update req
  const { projectId } = req.params;
...
export const remove = (req, res) => {
  //url 상으로 전달받은 클라이언트의 projectId delete req
  const { projectId } = req.params;
